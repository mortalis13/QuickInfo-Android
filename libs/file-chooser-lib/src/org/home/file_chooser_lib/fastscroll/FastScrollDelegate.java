package org.home.file_chooser_lib.fastscroll;
...
public class FastScrollDelegate {
  ...
  private FastScrollDelegate(final FastScrollable fastScrollable, int width, int height, Drawable thumbDrawable, boolean isDynamicHeight) {
    ...
    this.mThumbMinHeight = dp2px(height);
  ...
  public void setThumbSize(int widthDp, int heightDp) {
    this.mThumbWidth = dp2px(widthDp);
    this.mThumbHeight = dp2px(heightDp);
  ...
  private boolean onTouchEventInternal(MotionEvent event) {
    ...
    case MotionEvent.ACTION_DOWN:
      ...
      // DOWN and MOVE share the same thumb capture
      // (if the thumb activated by directly touching it or by moving pointer from the outside into the thumb)
      //break;
  ...
  private void onDrawScrollBars(Canvas canvas) {
    ...
    if (updateThumbRect(0)) {
      ...
      mThumbDrawable.setBounds(mThumbRect.left + scrollX, mThumbRect.top + scrollY, mThumbRect.right + scrollX, mThumbRect.bottom + scrollY);
  ...
  private boolean updateThumbRect(int touchDeltaY, boolean forceReportFastScrolled) {
    ...
    float visiblePercent = scrollExtent * 1f / scrollRange;
    // log("scrollPercent->" + scrollPercent + " visiblePercent->" + visiblePercent);
    
    final int viewHeight = mView.getHeight();
    final int viewWidth = mView.getWidth();
    
    // int thumbWidth = mThumbRect.width();
    int thumbWidth = this.mThumbWidth;
    // if (visiblePercent > 0.5f) thumbWidth = dp2px(FASTSCROLLER_DEFAULT_THUMB_WIDTH_DP);
    mThumbRect.right = viewWidth;
    mThumbRect.left = mThumbRect.right - thumbWidth;
    
    final int thumbHeight = mThumbDynamicHeight ? Math.max(mThumbMinHeight, Math.round(visiblePercent * viewHeight)) : mThumbMinHeight;
  ...
  public static int FASTSCROLLER_DEFAULT_THUMB_WIDTH_DP = 6;
  ...
  public static class Builder {
    ...
    public Builder radius(float radiusDp) {
      radius = dp2px(radiusDp);
      return this;
    }
    ...
    private Drawable makeDefaultThumbDrawable() {
      ...
      stateListDrawable.addState(DRAWABLE_STATE_PRESSED, pressedDrawable);
      
      GradientDrawable normalDrawable = new GradientDrawable();
      normalDrawable.setColor(thumbNormalColor);
      normalDrawable.setCornerRadius(radius);
      stateListDrawable.addState(DRAWABLE_STATE_DEFAULT, normalDrawable);
